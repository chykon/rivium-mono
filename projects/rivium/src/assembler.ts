...
function encodeRegister (regName: string): number {
...
export function assembly (sourceCode: string, core: vcore.Vcore): Uint8Array {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (command === 'addi') {
      // addi rd,rs1,iimm
      if (operands === undefined) throw Error('operands === undefined')
      const encodedOpcode = vcore.Opcode.OP_IMM
      const encodedRd = encodeRegister(operands[0] as string)
      const encodedFunct3 = 0b000
      const encodedRs1 = encodeRegister(operands[1] as string)
      const encodedIimm = core.valueToIimm(parseInt(operands[2] as string, 10))
      let instruction = 0
      ...
      const encodedRs1 = encodeRegister(operands[1] as string)
      let encodedIimm = core.valueToIimm(parseInt(operands[2] as string, 10))
      ...
      const encodedRd = encodeRegister(operands[0] as string)
      const encodedUimm = core.valueToUimm(parseInt(operands[1] as string, 10))
      let instruction = 0
      instruction |= encodedOpcode
      instruction |= encodedRd * (2 ** 7)
      ...
      const encodedFunct3 = 0b000
      const encodedRs1 = encodeRegister(operands[1] as string)
      const encodedRs2 = encodeRegister(operands[2] as string)
      ...
      const encodedRd = encodeRegister(operands[0] as string)
      const encodedJimm = core.valueToJimm(parseInt(operands[1] as string, 10))
      let instruction = 0
      instruction |= encodedOpcode
      instruction |= encodedRd * (2 ** 7)
      ...
      const encodedFunct3 = 0b000
      const encodedRs1 = encodeRegister(operands[0] as string)
      const encodedRs2 = encodeRegister(operands[1] as string)
      const encodedBimm = core.valueToBimm(parseInt(operands[2] as string, 10))
      let instruction = 0
      instruction |= encodedOpcode
      instruction |= encodedFunct3 * (2 ** 12)
      instruction |= encodedRs1 * (2 ** 15)
      ...
      const encodedRs1 = encodeRegister(operands[0] as string)
      const encodedRs2 = encodeRegister(operands[1] as string)
      const encodedSimm = core.valueToSimm(parseInt(operands[2] as string, 10))
      let instruction = 0
      instruction |= encodedOpcode
      instruction |= encodedFunct3 * (2 ** 12)
      instruction |= encodedRs1 * (2 ** 15)
